...
public class Move2Engine extends AbstractEngine
{
    ...
    public Move2Engine(Map<String, Integer> valuemap)
    {
        _valuemap = valuemap;
    }
    ...
    public String chooseMove(String position)
    {
        ...
        if (_numCenter == 1)
        {
            if (position.equals("f05"))
                consider.addAll(new HashSet<String>(Arrays.asList("f09", "c08",
                        "i08")));
            else if (position.equals("f07"))
                consider.addAll(new HashSet<String>(Arrays.asList("f03", "c04",
                        "i04")));
            else if (position.equals("e06"))
                consider.addAll(new HashSet<String>(Arrays.asList("i06", "h09",
                        "h03")));
            else
                consider.addAll(new HashSet<String>(Arrays.asList("c06", "d09",
                        "d03")));
        }
        else
        {
            ...
            for (String move : consider)
            {
                if (played.contains(move))
                    removal.add(move);
                else if (_numTop == 1 && atTop(move))
                    removal.add(move);
                else if (_numBottom == 1 && atBottom(move))
                    removal.add(move);
                else if (_numLeft == 1 && atLeft(move))
                    removal.add(move);
            ...
            if (_numCenterspoke == 1)
            {
                ...
                for (String move : _centerspoke)
                {
                    if (unplayed.contains(move)
                            && !isParallelNeighbor(move, first)
                            && !onStraightWith(move, first))
                    {
                        consider.add(move);
                    }
                }
